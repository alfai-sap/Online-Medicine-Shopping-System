...
public class LandingPageGUI {
    ...
    public LandingPageGUI(Inventory inventory, Cart cart) {
        ...
        this.cart = cart;
        User user = cart.getUser();  // The cart must have a user (Customer) assigned

        // Ensure that the cart is associated with the user properly
        
        if (user == null) {
            // Handle this scenario if user is not logged in
            JOptionPane.showMessageDialog(null, "Welcome! please login.");
            return;  // Prevent proceeding if no user is associated
        }
        ...
        viewOrderHistoryButton.addActionListener(e -> {
          // Get the associated user from the cart
            ...
            if (user instanceof Customer) {
                // Pass the valid customer instance to the OrderHistoryGUI
