...
public class AdminPanelGUI {
    ...
    private void viewOrdersDialog() {
        ...
        // Step 4: Loop through all users managed by UserManager
        for (User user : userManager.getAllUsers()) {
            // Step 5: Check if the user is a Customer (only customers have order histories)
            if (user instanceof Customer) {
                Customer customer = (Customer) user;
                // Step 6: Loop through all orders of the customer and add each order's details to the table model
                for (Order order : customer.getOrderHistory()) {
                    Object[] rowData = {
                        order.getOrderId(), // Order ID
                        customer.getName(), // Customer's name
                        order.getOrderDate(), // Date the order was placed
                        order.getStatus(), // Current status of the order (e.g., "Shipped")
                        String.format("PHP %.2f", order.getTotalAmount()) // Total amount formatted as PHP currency
                    };
                    tableModel.addRow(rowData); // Add the order data as a row in the table model
                }
            }
        }
    
        // Step 7: Create a JTable from the table model and add it to the dialog inside a JScrollPane (for scrolling)
        ...
        // Step 8: Create a close button to close the dialog
        JButton closeButton = new JButton("Close"); // Create a close button
        closeButton.addActionListener(e -> dialog.dispose()); // When clicked, dispose the dialog (close it)
        
        // Step 9: Add the close button to a new panel at the bottom of the dialog
        JPanel buttonPanel = new JPanel(); // Create a new panel for the buttons
        buttonPanel.add(closeButton); // Add the close button to the panel
        
        // Step 10: Add the button panel to the bottom of the dialog
        dialog.add(buttonPanel, BorderLayout.SOUTH); // Add the button panel to the south region of the dialog
    
        // Step 11: Center the dialog on the screen relative to the main frame
        dialog.setLocationRelativeTo(frame);
    
        // Step 12: Make the dialog visible to the user
        dialog.setVisible(true);
    }
  ...
  private void refreshInventoryDisplay() {
    ...
    private void updateMedicineDialog() {
        ...
        // Repeat for other fields (Price, Stock, Description, Components, Side Effects, Image Path)
        gbc.gridx = 0; gbc.gridy = 1; panel.add(new JLabel("New Price (optional):"), gbc); gbc.gridx = 1; panel.add(priceField, gbc);
        gbc.gridx = 0; gbc.gridy = 2; panel.add(new JLabel("New Stock (optional):"), gbc); gbc.gridx = 1; panel.add(stockField, gbc);
        gbc.gridx = 0; gbc.gridy = 3; panel.add(new JLabel("New Description (optional):"), gbc); gbc.gridx = 1; panel.add(descriptionField, gbc);
        gbc.gridx = 0; gbc.gridy = 4; panel.add(new JLabel("New Components (optional):"), gbc); gbc.gridx = 1; panel.add(componentsField, gbc);
        gbc.gridx = 0; gbc.gridy = 5; panel.add(new JLabel("New Side Effects (optional):"), gbc); gbc.gridx = 1; panel.add(sideEffectsField, gbc);
        gbc.gridx = 0; gbc.gridy = 6; panel.add(new JLabel("New Image Path (optional):"), gbc); gbc.gridx = 1; panel.add(imagePathField, gbc);
